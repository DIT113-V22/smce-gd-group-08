...
extends RayCast
class_name WorkWheel
func extern_class_name():
	return "WorkWheel"

# change here .y to to.z
var max_hit_distance: float = abs(cast_to.z)
...
# change here Vector3
export var force_offset = Vector3(0,0,-0.2)
...
func add_force(state: PhysicsDirectBodyState) -> void:
	# change here to.y to to.z
	max_hit_distance = abs(cast_to.z)
...
#   change here point.z to point.y
...
func _friction_force(state: PhysicsDirectBodyState) -> void:
	...
	var v: Vector3 = (global_transform.origin - prev_pos) / _hit_delta
	# change here .z to .y
	var speed: float = sqrt(v.x * v.x + v.y * v.y)
	...
	var Crr: float = 0.05
	# change vector here 
	var frolling: Vector3 = -Crr * v * Vector3(1, 1, 0)
	...
	# change vector3.forward here with vector3.UP
	var brr = v.normalized().angle_to(_parent.transform.basis.xform(Vector3.UP))
	if abs(throttle) > 0:
